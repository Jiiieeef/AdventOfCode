...
const mapping = {
  [NORTH]: (coordinates, value) => ({...coordinates, Y: coordinates.Y + value}),
  [SOUTH]: (coordinates, value) => ({...coordinates, Y: coordinates.Y - value}),
  [EAST]: (coordinates, value) => ({...coordinates, X: coordinates.X + value}),
  [WEST]: (coordinates, value) => ({...coordinates, X: coordinates.X - value})
};

const splitInstruction = (instruction) => {
  const value = instruction.match(/[0-9]+$/g)[0];
  const letter = instruction[0];

  return [letter, +value];
};

const rotateShip = (letter, value) => {
