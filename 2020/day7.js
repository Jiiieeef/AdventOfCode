const input = `shiny purple bags contain 2 pale blue bags, 1 wavy fuchsia bag, 5 pale salmon bags.
...
const bagsContent = inputArray.map(row => {
  const [color, content] = row.split(' contain ');
...
const findContainersForBag = (bagname) => {
  bagsContent.forEach((bagContent) => {
    if (bagContent.content.find(content => content.includes(bagname))) {
      bags.push(bagContent.color);

      findContainersForBag(bagContent.color.replace(/ (bags|bag)/, ''));
    }
  });
};

findContainersForBag('shiny gold');

console.log(`Part 1: ${[...new Set(bags)].length}`);

const findNumberOfBags = (bagname) => {
  const match = bagname.match(/^[0-9]{1,2}/);
  return match && match[0];
};


const countTotalContentForBag = (bagname) => {
  return bagsContent.reduce((sum, bagContent) => {
    if (bagContent.color.includes(bagname)) {

      let currentCount = 0;
      bagContent.content.forEach(content => {
        const number = findNumberOfBags(content);

        if (number) {
          const currentBagName = content.replace(number, '').trim();

          currentCount += number * countTotalContentForBag(currentBagName.replace(/ (bags|bag)/, ''));
        }
      });
...
// -1 beacause shiny gold bar must not be count
const total = countTotalContentForBag('shiny gold') - 1;
